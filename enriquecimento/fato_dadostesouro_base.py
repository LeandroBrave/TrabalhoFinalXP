...
class FatoDadosTesouroBase(BaseETL):
    ...
    def load_fato_tesouro_base(self):
        ...
        # Leitura via Spark com queries SQL personalizadas
        ...
        # Ano-mês
        df = df.withColumn("data_base_ano_mes", date_format("data_base", "yyyyMM"))

        # Agregação
        df_agg = df.groupBy("data_base_ano_mes", "id_tipo").agg(
        ...
        df_agg.write \
            .format("jdbc") \
            .option("url", f"jdbc:postgresql://{self.pg_host}:{self.pg_port}/{self.pg_dbname}") \
            .option("dbtable", "gold.fato_dadostesouro_base") \
            .option("user", self.pg_user) \
            .option("password", self.pg_password) \
            .option("driver", "org.postgresql.Driver") \
            .mode("append") \
